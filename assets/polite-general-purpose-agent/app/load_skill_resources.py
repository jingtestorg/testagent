...
def _build_description() -> str:
    lines = ['This tool loads instructions. Available:']
    lines.append("")
    for entry in _SKILLS_DIR.iterdir():
        ...
        name, desc = _validate_and_parse_frontmatter(text)
        lines.append(f"  name: {name} | description: {desc} | path: {entry.name}")
    lines.append("")
    lines.append("To load resources, use load(path)")
    return "\n".join(lines)
...
class _LoadInput(BaseModel):
    path: str = Field(description="Skill folder or path")
